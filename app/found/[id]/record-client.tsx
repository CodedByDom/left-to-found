...
export default function RecordClient({
  ...
  const [phase, setPhase] = useState<"idle" | "form" | "submitting">(
    "idle"
  );
  ...
  const isFound =
    current.status === "found" || current.found === true;

  const dropDate = formatDate(
    current.dropped_at || current.hidden_date || null
  );

  const foundDate = formatDate(
    current.found_at || current.found_date || null
  );

  const finderLoc = [
    current.finder_location || current.location,
  ...
  const timeOutThere = useMemo(
    () => daysBetween(current.dropped_at, current.found_at),
    ...
    try {
      const res = await fetch("/api/found", {
        ...
        body: JSON.stringify({
          ...
          finderCountry: finderCountry.trim(),
      ...
      {!isFound ? (
        <section className="finder-panel">
          <p className="finder-intro">
            This photograph is somewhere out in the world.
            If you&apos;re holding it, you can become part
            of its record.
          </p>
          ...
          {(phase === "form" ||
            ...
            <div className="form-area fade-in">
              <div className="form-grid">
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFinderLocation(e.target.value)
                    }
                    disabled={phase === "submitting"}
                  />
                </label>
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    value={finderCountry}
                    onChange={(e) =>
                      setFinderCountry(e.target.value)
                    }
                    disabled={phase === "submitting"}
                  />
                </label>
              ...
              <p className="privacy-note">
                Your note may be shown on this page after
                review. Everything is optional.
              </p>
        ...
        <section className="found-panel">
          ...
          {confirmed &&
            ...
            !current.finder_message_public && (
              <p className="privacy-note">
                Thanks for leaving a note. It will appear
                here if approved.
              </p>
            )}
        </section>
